...
def plot_frame(project, frame_num, detector_names=None, markersize=8, figsize=[15, 10], confidence_thr=0.75):
    ...
    for model in detector_names:
        ...
        # matched_id = int(image[0].split('_')[-1].split('.')[0])
        matched_id = int(image[0].split('/')[-1].split('_')[0][4:])
        infile = os.path.join(project, 'detection', model + '_evaluation', 'performance_detection.json')
        with open(infile) as jsonfile:
            cocodata = json.load(jsonfile)
        print(matched_id)
        ...
        # image_ids = []
        # for i in cocodata['pred_bbox']:
        #     image_ids.append(i['image_id'])
        # print(np.unique(np.array(image_ids)))
        # print(len(image_ids))
        # pred = [i for i in cocodata['pred_bbox'] if 0 == 1]
        # gt = [i for i in cocodata['gt_bbox']['annotations'] if 0 == 1]
        ...
        for rank, pt in enumerate(pred):
            ...
            if rank == 0:
                ...
                legend_flag[2] = True
        # for rank, pt in enumerate(pred):
        #     x = np.array([pt['bbox'][0], pt['bbox'][0]]) * dims[1]
        #     y = np.array([pt['bbox'][1], pt['bbox'][1]]) * dims[0]
        #     print(x)
        #     print(y)
        #     if rank == 0:
        #         plt.plot([x[0], x[1], x[1], x[0], x[0]], [y[0], y[0], y[1], y[1], y[0]], color=colors[1],
...
def plot_training_progress(project, detector_names=None, figsize=(14, 6), logTime=False, omitFirst=0):
    ...
    for i, model in enumerate(detector_names):
        ...
        print('sm_vals at min')
        pprint(sm_vals[np.where(steps == min_step)])
        print('sm_vals')
        pprint(sm_vals)
